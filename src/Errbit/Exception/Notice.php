...
class Notice
{
     ...
     */
    public function __construct(private readonly mixed $exception, array $options = [])
    {
        $this->options   = array_merge(
            ['url'          => $this->buildRequestUrl(), 'parameters'   => !empty($_REQUEST) ? $_REQUEST : [], 'session_data' => !empty($_SESSION) ? $_SESSION : [], 'cgi_data'     => !empty($_SERVER)  ? $_SERVER  : []],
            $options
        );
     ...
     * @return Notice
     */
    public static function forException(mixed $exception, array $options = []): Notice
    {
     ...
     *
     * @param \Errbit\Utils\XmlBuilder $builder the builder instance to set the data into
                        ...
                        $error->tag(
                            ...
                            function (XmlBuilder $backtrace) use ($exception, $self) {
                                ...
                                $backtrace->tag(
                                    'line',
                                    '',
                                    ['number' => $exception->getLine(), 'file' => !empty($file1) ? $self->filterTrace($file1) : '<unknown>', 'method' =>  "<unknown>"]
                                );
                                ...
                                if (empty($trace)) {
                                    ...
                                    foreach ($trace as $frame) {
                                        $backtrace->tag(
                                            'line',
                                            '',
                                            ['number' => $frame['line'] ?? 0, 'file'   => isset($frame['file']) ?
                                                $self->filterTrace($frame['file']) : '<unknown>', 'method' => $self->filterTrace($self->formatMethod($frame))]
                                        );
                                    }
                                }
                            }
...
//                        $env->tag('hostname', $options['hostname']);
    ...
    {
        return ['namespace' => array_slice(explode('\\', $name), 0, -1), 'classname' => implode('', array_slice(explode('\\', $name), -1))];
    }
}
